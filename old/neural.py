...
# Установите бэкенд Matplotlib
matplotlib.use("TkAgg")

# Определение класса для преобразования изображения в STL
class ImageToSTLConverter:
    def __init__(self, scale=1.0, threshold=100):
        self.scale = scale
        self.threshold = threshold

    def image_to_stl(self, image_path, stl_path):
        # Реализация преобразования изображения в STL
        print(f"Converting {image_path} to {stl_path}")
        # Добавьте код преобразования здесь
...
def main(nu, length, width, volfrac, penal, rmin, ft):
    ...
    fig, ax = plt.subplots()
    fig.set_size_inches(10, 10)  # Установить размер фигуры
